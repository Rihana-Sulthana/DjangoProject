...
class MovieRating(APIView):
    ...
    def post(self, request):
        try:
            ...
            movie_obj = Movierating.objects.update_or_create(
                rating=data['rating'],
                defaults={"user_id": user, "movie": data['movie']}
            )
            return Response("Rating Added or Updated Successfully")
    ...
    def get(self, request):
        try:
            user = request.user.id
            movies = list(Movierating.objects.filter(user_id=user).values('movie', 'rating'))
    ...
    def delete(self, request):
        try:
            user = request.user.id
            movies = Movierating.objects.filter(user_id=user).delete()
            return Response("User Ratings Deleted Successfully")
